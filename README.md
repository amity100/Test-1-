...
1. **Build phase** (3, 5, 8 minutes or unlimited). Place blocks, box-fill, stamp prefabs (walls,
   towers, gates, arches, stairs, domes, roofs, bridges, pyramids), paint, mirror, undo/redo. Six
   style palettes: Candy, Gothic, Modern, Medieval, Neon Cyber, Desert Adobe. Save and load
   blueprints.
2. **Hide the flag.** The flag must remain reachable: a flood fill from outside the plot (walking,
   ...
   rooms are rejected.
3. **Rounds** (4 minutes each, one per fortress). The owner defends; attackers fight each other too.
   Standing three seconds next to the flag captures it and ends the round. When time runs out the
   defender earns a hold bonus.
...
greedy meshing with baked vertex AO, DataArrayTexture PBR materials), heightmap island, height fog
post effect, procedural Web Audio, bots with A* over a walkability grid.

## Tests

`node scripts/smoke.mjs <url>` and `node scripts/smoke-game.mjs <url>` drive the game in headless
Chromium (Playwright) and save screenshots.
