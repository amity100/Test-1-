...
export class HiddenBlade {
  ...
  showT = 0;
  /** Closing in (a lunge, or a step): at whom, and how long it may still take (s). */
  lunging: { enemy: EnemyView; t: number } | null = null;
  ...
  pick(feet: V3, facing: V3, look: V3): BladeTarget | null {
    ...
    for (const e of this.h.enemies) {
      ...
      // a lunge only at a man ahead of you, about level, and not flying past
      if (!reach && (ahead < BLADE.lungeCone || rise > BLADE.lungeRise || e.state === 'launched')) continue;
  ...
  start(e: EnemyView, feet: V3): boolean {
    ...
    const run = Math.hypot(e.pos.x - feet.x, e.pos.z - feet.z) - e.radius - BLADE.lungeStop;
    if (run <= 0) return true;
    this.lunging = { enemy: e, t: Math.min(BLADE.lungeTime, run / BLADE.lungeSpeed + 0.1) };
    return false;
  }
   ...
   * up with him still in reach), 'miss' when he got away or died on the way.
   */
  update(dt: number, feet: V3, dir: V3): LungeStep | null {
    ...
    const dx = e.pos.x - feet.x, dz = e.pos.z - feet.z;
    const d = Math.hypot(dx, dz);
    if (e.alive && L.t > 0 && d > e.radius + BLADE.lungeStop) {
      dir.set(dx / d, 0, dz / d);
      return 'go';
    }
    this.lunging = null;
    return e.alive && d <= BLADE.reach + e.radius && Math.abs(e.pos.y - feet.y) <= BLADE.reachRise ? 'strike' : 'miss';
  }
  ...
  /** Nothing solid between you (fences too); a lunge also needs a clear run and a floor to it. */
