...
import { cooling, firstSearchers, talkRound } from './searchers';
...
export const TEACH = [
  {
    ...
    body: 'שני צעדים, תמיד. **להיכנס** — ומיד בערך חצי מהמקום שלי. '
  ...
  {
    id: 'power', title: 'ידיים: כמה דברים אני מספיק לעשות בבת אחת',
    body: 'יש לי כמה ידיים, וכל דבר שאני עושה תופס יד אחת או יותר עד שהוא נגמר. '
      + '**יד לא נגמרת — היא תפוסה**, וברגע שמשהו נגמר או שאני עוצר אותו, היד חוזרת אליי. '
      + 'רוצים להתחיל משהו חדש כשכל הידיים תפוסות? עוצרים משהו אחר. '
      + 'וכל חברה שכולה שלי נותנת לי עוד יד.',
  ...
  },
  {
    ...
    body: 'כשתחנת החשמל של אזור שלי, כל מה שאני עושה באזור שלה קל יותר — וזה כתוב על כל מקום. '
      + 'לבחור **באיזה סדר** לקחת מקומות — זה החלק העמוק של המשחק.',
...
export function tick(state: GameState, mins: number) {
  ...
  coolOff(state);
  maybeJoin(state);
  // And the country itself: whoever saw something today tells whoever they
  // would really tell, and whoever nobody fed for a day and a half lets go.
  talkRound(state);
  cooling(state, mins);
  opinionDay(state);
  grow(state);
